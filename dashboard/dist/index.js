...
class Model {
  constructor() {
    ...
    for (let [target, [command, opts]] of Object.entries(dashQueries))
      this.horizon.utensils.sendRequest(`admin:${command}`, opts)
  ...
  browserPrev() {
    ...
    this.horizon.utensils.sendRequest("query", query).toArray().forEach(output => {
  ...
  onBrowserStateChange(change, old) {
    ...
    this.horizon.utensils.sendRequest("subscribe", change.query.toJS())
