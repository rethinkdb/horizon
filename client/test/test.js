...
var aboveSubscriptionSuite = function aboveSubscriptionSuite(getData) {
  return function () {
    var data = undefined;
    ...
    it('can get a specific document', assertCompletes(function () {
    ...
    // `changed` event.
    it('can get a document and reflect changes to it', assertCompletes(function () {
      return observableInterleave({
        query: data.above({ id: 1 }).watch(),
    ...
    it('can get a document by secondary index with open bound', assertCompletes(function () {
      return observableInterleave({
    ...
    it('doesn\'t see updates to documents outside its bound', assertCompletes(function () {
      return observableInterleave({
...
var chainingSuite = function chainingSuite(getData) {
  return function () {
    var data = undefined;
    ...
    it('findAll.order.above.below', assertCompletes(function () {
      return data.findAll({ a: 20 }).order('id').above({ id: 2 }).below({ id: 4 }).fetch({ asCursor: false }).do(function (res) {
        return assert.deepEqual(res, [{ id: 2, a: 20, b: 1 }, { id: 3, a: 20, b: 2 }]);
      });
    }));

    // Let's flip it the other way and change the order
    it('findAll.below.above.order(desc)', assertCompletes(function () {
      return data.findAll({ a: 20 }).below({ id: 4 }).above({ id: 2 }).order('id', 'descending').fetch({ asCursor: false }).do(function (res) {
        return assert.deepEqual(res, [{ id: 3, a: 20, b: 2 }, { id: 2, a: 20, b: 1 }]);
      });
    }));

    // Let's throw limit into the mix
    it('findAll.order.above.below.limit', assertCompletes(function () {
      return data.findAll({ a: 20 }).above({ id: 2 }).order('id').below({ id: 4 }).limit(1).fetch({ asCursor: false }).do(function (res) {
        return assert.deepEqual(res, [{ id: 2, a: 20, b: 1 }]);
      });
    }));

    // Let's do it on the collection
    it('order.above.below.limit', assertCompletes(function () {
      return data.below({ id: 4 }).order('id').above({ id: 2 }).limit(1).fetch({ asCursor: false }).do(function (res) {
        return assert.deepEqual(res, [{ id: 2, a: 20, b: 1 }]);
      });
    }));
    ...
    it('findAll.order.above.below.limit', assertCompletes(function () {
      return data.findAll({ a: 20 }).order('id').above({ id: 2 }).below({ id: 4 }, 'closed').limit(2).fetch({ asCursor: false }).do(function (res) {
        return assert.deepEqual(res, [{ id: 2, a: 20, b: 1 }, { id: 3, a: 20, b: 2 }]);
      });
    }));

    // Can't chain off vararg `findAll`
    it('throws if order is chained off a multi-arg findAll', assertThrows('order cannot be called on the current query', function () {
...
var findAllSubscriptionSuite = function findAllSubscriptionSuite(getData) {
  return function () {
    var data = undefined;
    ...
    // Let's grab a specific document using `findAll`
    it('can find a single document', assertCompletes(function () {
      return observableInterleave({
    ...
    // event.
    it('can find a document and reflect changes', assertCompletes(function () {
      return observableInterleave({
        query: data.findAll(1).watch(),
        operations: [data.store({ id: 1, a: 1 }), data.store({ id: 1, a: 2 }), data.remove(1)],
        expected: [[], [{ id: 1, a: 1 }], [{ id: 1, a: 2 }], []]
    ...
    // Let's make sure we don't see events that aren't ours
    it('doesn\'t see changes to documents outside its range', assertCompletes(function () {
      return observableInterleave({
        query: data.findAll(1).watch(),
    ...
    it('can subscribe to multiple ids', assertCompletes(function () {
      return observableInterleave({
        ...
        operations: [data.store({ id: 1, a: 1 }), data.store({ id: 2, a: 1 }).concat(data.store({ id: 3, a: 1 })), data.store({ id: 1, a: 2 }), data.store({ id: 2, a: 2 }).concat(data.store({ id: 3, a: 2 })), data.remove(1), data.remove(2).concat(data.remove(3))],
        expected: [[], [{ id: 1, a: 1 }], [{ id: 1, a: 1 }, { id: 2, a: 1 }], [{ id: 1, a: 2 }, { id: 2, a: 1 }], [{ id: 1, a: 2 }, { id: 2, a: 2 }], [{ id: 2, a: 1 }], []]
      });
    }));

    // Let's make sure initial vals works correctly
    it('properly handles initial values', assertCompletes(function () {
      return data.store({ id: 1, a: 1 }).concat(observableInterleave({
        query: data.findAll(1).watch(),
        operations: [data.store({ id: 1, a: 2 }), data.remove(1)],
        expected: [[{ id: 1, a: 1 }], [{ id: 1, a: 2 }], []]
...
var findSubscriptionSuite = function findSubscriptionSuite(getData) {
  return function () {
    var data = undefined;
    ...
    it('returns an updating document', assertCompletes(function () {
      return observableInterleave({
    ...
    // Let's grab a specific document using `find`
    it('receives results from store operations', assertCompletes(function () {
      return observableInterleave({
        query: data.find(1).watch(),
        operations: [data.store({ id: 1, a: 1 }), data.store({ id: 1, a: 2 }), data.remove(1)],
    ...
    // Let's make sure we don't see events that aren't ours
    it('doesn\'t see events that don\'t belong to it', assertCompletes(function () {
      return observableInterleave({
        query: data.find(1).watch(),
...
var insertSuite = function insertSuite(getData) {
  return function () {
    var data = undefined;
    ...
    it('stores documents in db, errors if documents already exist', assertErrors(function () {
      ...
      .do(function (res) {
        return assert.deepEqual({ id: 1, a: 1, b: 1 }, res);
      })
    ...
    it('generates ids if documents don\'t already have one', assertErrors(function () {
      ...
      // Check that we get back what we put in.
      .do(function (res) {
        return assert.deepEqual({ id: new_id, a: 1, b: 1 }, res);
      })
    ...
    it('fails if null is passed', assertThrows('The argument to insert must be non-null', function () {
      return data.insert(null);
    }));

    it('fails if undefined is passed', assertThrows('The 1st argument to insert must be defined', function () {
      return data.insert(undefined);
    }));

    it('fails if given no argument', assertThrows('insert must receive exactly 1 argument', function () {
      return data.insert();
    }));
    ...
    it('can store multiple documents in one call', assertCompletes(function () {
      var new_id_0 = undefined,
          new_id_1 = undefined;
      ...
      .flatMap(function () {
        return data.findAll(new_id_0, new_id_1, 1).fetch({ asCursor: false });
      })
      // We're supposed to get an array of documents we put in
      .do(function (res) {
        return assert.sameDeepMembers(res, [{ id: new_id_0 }, { id: new_id_1, a: 1 }, { id: 1, a: 1 }]);
      });
    }));

    // If any operation in a batch insert fails, everything is reported as a
    // failure.
    it('fails if any operation in a batch fails', assertErrors(function () {
      return(
...
var replaceSuite = function replaceSuite(getData) {
  return function () {
    var data = undefined;
    ...
    it('replaces an existing document completely', assertCompletes(function () {
      ...
      .do(function (res) {
        return assert.deepEqual(res, { id: 1, a: { c: 2 } });
      });
    }));

    // The `replace` command replaces documents already in the database. It
    // errors if the document doesn't exist.
    it('fails if the document does not already exist', assertErrors(function () {
      return data.replace({ id: 1, a: 1, b: 1 });
    }));

    // It means you can't replace a document without providing an id.
    it('fails if document does not have an id', assertErrors(function () {
      return data.replace({ a: 1, b: 1 });
    }));

    // Calling `replace` with `null` is an error.
    it('fails if null is passed', assertThrows('The argument to replace must be non-null', function () {
      return data.replace(null);
    }));

    // Calling `replace` with `undefined` is also an error.
    it('fails if undefined is passed', assertThrows('The 1st argument to replace must be defined', function () {
      return data.replace(undefined);
    }));

    it('fails if passed no arguments', assertThrows('replace must receive exactly 1 argument', function () {
      return data.replace();
    }));
    ...
    it('allows replacing multiple documents with one call', assertCompletes(function () {
      ...
      .flatMap(data.replace([{ id: 1, a: { c: 2 } }, { id: 2, d: 3 }])).toArray()
      // We should have gotten the ID back again
      .do(function (res) {
        return assert.deepEqual(res, [1, 2]);
      })
      ...
      // Check that we get back what we put in.
      .do(function (res) {
        return assert.sameDeepMembers(res, [{ id: 1, a: { c: 2 } }, { id: 2, d: 3 }]);
      });
    }));
    ...
    it('fails if any document in a batch fails to be replaced', assertErrors(function () {
      return data.replace([{ id: 1, a: 1 }, null, { a: 1 }]);
    }));

    // Replacing an empty batch of documents is ok, and returns an empty
    // array.
    it('allows an empty batch of documents', assertCompletes(function () {
...
var storeSuite = function storeSuite(getData) {
  return function () {
    var data = undefined;
    ...
    it('creates or replaces a document', assertCompletes(function () {
      ...
      // Check that we get back what we put in.
      .do(function (res) {
        return assert.deepEqual(res, { id: 1, a: 1, b: 1 });
      })
      ...
      // Check that we get back what we put in.
      .do(function (res) {
        return assert.deepEqual(res, { id: 1, c: 1 });
      });
    }));
    ...
    it('generates ids for documents without them', assertCompletes(function () {
      var new_id = undefined;
      ...
      // We should have gotten the ID back again
      .do(function (res) {
        return assert.deepEqual(new_id, res);
      })
      // Make sure `store` overwrote the original document
      .flatMap(function () {
        return data.find(new_id).fetch();
      ...
      // Check that we get back what we put in.
      .do(function (res) {
        return assert.deepEqual({ id: new_id, c: 1 }, res);
      });
    }));

    // Storing `null` is an error.
    it('fails if null is passed', assertThrows('The argument to store must be non-null', function () {
      return data.store(null);
    }));

    // Storing `undefined` is also an error.
    it('fails if undefined is passed', assertThrows('The 1st argument to store must be defined', function () {
      return data.store(undefined);
    }));

    // Storing nothing is an error
    it('fails if no arguments are passed', assertThrows('store must receive exactly 1 argument', function () {
      return data.store();
    }));
    ...
    it('allows storing multiple documents in one call', assertCompletes(function () {
      var new_id_0 = undefined,
          new_id_1 = undefined;
    ...
    // array.
    it('allows storing empty batches', assertCompletes(function () {
    ...
    it('stores date objects and retrieves them again', assertCompletes(function () {
      var originalDate = new Date();
      return data.store({ date: originalDate }).toArray().flatMap(function (id) {
...
var upsertSuite = function upsertSuite(getData) {
  return function () {
    var data = undefined;
    ...
    // them if they already exist.
    it('updates existing documents or creates them if they don\'t exist', assertCompletes(function () {
    ...
    it('generates ids for documents without them', assertCompletes(function () {
      ...
      // Check that we get back what we put in.
      .do(function (res) {
        return assert.deepEqual(res, { id: new_id, a: { b: 1, c: 1 }, d: 1 });
      })
      // Let's update the document now
      .flatMap(function () {
      ...
      // We should have gotten the ID back again
      .do(function (res) {
        return assert.deepEqual(res, [new_id]);
      })
      // Make sure `upsert` updated the original document
      .flatMap(function () {
        return data.find(new_id).fetch();
      ...
      // Check that we get back what we put in.
      .do(function (res) {
        return assert.deepEqual(res, { id: new_id, a: { b: 1, c: 2 }, d: 1 });
      });
    }));

    // Upserting `null` is an error.
    it('fails if null is passed', assertThrows('The argument to upsert must be non-null', function () {
      return data.upsert(null);
    }));

    // Upserting `undefined` is also an error.
    it('fails if undefined is passed', assertThrows('The 1st argument to upsert must be defined', function () {
      return data.upsert(undefined);
    }));

    it('fails if no arguments are passed', assertThrows('upsert must receive exactly 1 argument', function () {
      return data.upsert();
    }));
    ...
    it('allows upserting multiple documents in one call', assertCompletes(function () {
      var new_id_0 = undefined,
          new_id_1 = undefined;
    ...
    // as a failure.
    it('fails if any document in the batch fails ', assertErrors(function () {
      return data.upsert([{ a: 1 }, null, { id: 1, a: 1 }]);
    }));

    // Upserting an empty batch of documents is ok, and returns an empty
    // array.
    it('allows upserting an empty batch', assertCompletes(function () {
